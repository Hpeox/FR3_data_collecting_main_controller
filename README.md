...
当物理 Xense 不可用而需要验证主控、RealSense 与 rosbag 工作流时，可重复使用
`--mock xense`。mock Xense 仍作为受管 Xense 进程运行，并沿用完整的 LocalStore、
SHM、UDS、`data_TAC_*.npy`、tactile QC 与 preview 流程；只有 SDK worker 和物理硬件
初始化被替换为确定性的零填充 tensor。当前只支持 `xense`；重复传入会去重。
...
- `demos/mock_demo_YYYYmmdd_HHMMSS/`：包含至少一个 mock component 的单次 demo；其
  内容仍包含完整的主控、Xense tactile 与 RealSense/rosbag 输出。该前缀用于自动隔离
  非正式数据，DatasetBuilder 当前的 `glob("demo_*")` 不会遍历它。
