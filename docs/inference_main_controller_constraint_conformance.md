...
| 11. SensorHub fatal surfaces through LeRobot and terminates the worker/session | LeRobot / SensorHub | External blocked | `FR3._check_health()` consumes SensorHub fatal during FR3 operations, so RUNNING propagation is covered by normal observation reads. The controlled worker does not poll FR3 health while blocked in an idle control phase. MainController logs aligned fatal diagnostically and waits for the LeRobot fatal path; it does not assume SensorHub propagation ownership. No external code is changed here. |
...
| 19. SHUTDOWN is user-only, state-valid, non-deferred, and permits return-home; every system fault maps to no-new-motion FAIL_STOP | MainController / LeRobot | Satisfied | Admission accepts user `shutdown` in `WAIT_START`, `READY`, or `RUNNING`; RUNNING performs immediate `STOP` and recording finalization before `SHUTDOWN`. It is rejected during blocking `INITIALIZE`. Faults call `request_fail_stop()`, including promotion of an in-progress graceful termination when a new fault is observed. External worker separates `_graceful_shutdown()` return-home from `_fatal_stop()`. |
...
| 25. Explicit v1 non-goals remain absent | MainController | Satisfied | Final source audit found no ordinary queue, deferred command/shutdown latch, `q_reset`, NUC/SensorHub supervisor, process-restart call, reset-preemption mechanism, raw-`RESETTING` completion, or full aligned payload read. Focused negative tests cover the highest-risk boundaries. |
| 26. Core invariants hold together | Shared boundary | Satisfied with external items above | MainController focused/core tests, existing collection mock regression, external LeRobot Controlled tests, compile checks, and this final row-by-row audit completed. The two explicitly marked external API/idle-propagation gaps remain visible rather than being moved into MainController. |
...
- `/usr/bin/python3 -m pytest -q MainController/src/main_controller/test/test_inference_protocol.py MainController/src/main_controller/test/test_inference_controller.py MainController/src/main_controller/test/test_maincontroller_core.py`: 90 passed.
...
- `conda run -n lerobot-fr3-312 env PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 HF_HOME=/tmp/codex_lerobot_hf_cache python -m pytest -q tests/test_controlled_rollout.py`: 20 passed outside the restricted socket sandbox.
- `/usr/bin/python3 -m compileall -q main_controller` and `git diff --check`: passed.
